...
const posts = computed(() => {
  return (props.posts || site.postList).filter(post => !isHiddenInIndex(post))
})
...
function isHiddenInIndex(post: Post) {
  const hide = post.hide as unknown
  return hide === true || hide === 'index' || hide === 'all'
}
</script>
